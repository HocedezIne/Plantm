...
const drawChart = function(labels, data){
  let ctx = document.querySelector(".js-smoist-chart").getContext("2d");
...
const init = function () {
  console.log('smoist init geladen');
  listenToSocket();
  listenToUI();
  showCurrentDates();
  socket.emit("F2B_send_min_max", {deviceid: "BV"})
  socket.emit("F2B_send_data", {data: dates})
};
